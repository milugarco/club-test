...
import { HiOutlineCloudUpload, HiPencil, HiTrash } from 'react-icons/hi'
...
const GuestStepSix = (props: GuestFormPropsNew) => {
    ...
    const validateInputs = () => {
        ...
        //   if (!showFormatSign) { // Inicie a animação somente se showFormatSign for falso
        ...
        //         // Após 3 segundos, chame handleStep
        //         setTimeout(() => {
        //           console.log('Nine--Ten', { ...formData, ...formValueStep });
        ...
        <div className=" flex flex-col justify-center items-center gap-4">
            ...
            <div className="flex flex-col text-sm dark:text-white text-center">
                <p className="assinatura-user-name-terms text-2xl sm:text-4xl">
                    {formValueStep?.newEventParticipant?.user?.name}
                </p>

                <hr />
                <p>{formValueStep?.newEventParticipant?.user?.name}</p>
                <p>{formValueStep?.documentNumber}</p>
            </div>

            <Button
                className=" w-full md:w-60 shadow-lg mt-4 flex flex-row gap-2 justify-center items-center"
                variant="solid"
                onClick={() => validateInputs()}
                disabled={isSigned} // Desabilita o botão se ele estiver assinado
            >
                {isSigned ? (
                    <>
